...
export interface LoadContentOptions {
  ...
  /** Content type filter */
  type?: string;
  /** Whether to include drafts (defaults to ORICMS_PREVIEW env var or false) */
  preview?: boolean;
  /** Limit results */
  limit?: number;
}
...
/**
 * Load content from a OriCMS repository
 */
...
export async function loadPage(contentPath: string, slug: string, preview?: boolean): Promise<Page | null> {
  ...
  return pages.find(p => p.metadata.slug === slug || p.$id === slug) || null;
}

/**
 * Load the primary (root) entry of a collection
 */
export async function loadCollectionRoot(contentPath: string, collectionId: string): Promise<Page | null> {
  try {
    ...
    const config = collections.find(c => c.id === collectionId);

    if (config?.routing?.homepageId) {
      return loadPage(contentPath, config.routing.homepageId);
    }
  } catch (error) {
    console.warn(`Failed to load collection root for ${collectionId}:`, error);
  }
  return null;
}
    ...
    for (const file of files) {
      try {
        ...
        const parsed = JSON.parse(content) as {
          ...
          label?: string;
          description?: string;
          fields?: Array<SchemaField & { relation?: Record<string, unknown> }>;
        };
        schemas.push({
        ...
        console.warn(`Failed to parse schema ${file}:`, error);
      }
    }
