...
export async function createContentType(
  ...
  await dispatchLifecycleEvent('contentType.beforeCreate', {
  ...
  await deps.gitService.writeFile(context.projectId, path, content, {
    message: `Create content type: ${contentType.label}`,
  ...
  await dispatchLifecycleEvent('contentType.afterCreate', {
    projectId: context.projectId,
    typeId: contentType.name,
    actor: context.actor,
    contentType: contentType as unknown as Record<string, unknown>,
  ...
  apiServices.runBackgroundTask(
    'plugin-hook:content-type-create',
    pluginDispatcher({
      projectId: context.projectId,
      event: PLUGIN_EVENT_NAMES.CONTENT_TYPE_CREATED,
      resourceType: 'contentType',
      resourceId: contentType.name,
  ...
  queueDeliveryProjectionReconcile({
    projectId: context.projectId,
    label: 'delivery-projection:content-type-create',
  });
