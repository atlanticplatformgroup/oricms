...
export const collectionsApi = {
  async list(projectId: string): Promise<{ collections: CollectionConfig[] }> {
    return request(`/api/v1/projects/${projectId}/collections`);
  },

  async updateConfig(projectId: string, collections: CollectionConfig[], headers?: Record<string, string>): Promise<void> {
    return request(`/api/v1/projects/${projectId}/collections`, { method: 'PUT', body: { collections }, headers });
  },

  async deleteCollection(projectId: string, collectionId: string, headers?: Record<string, string>): Promise<void> {
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}`, { method: 'DELETE', headers });
  },
  ...
  async listEntries(projectId: string, collectionId: string, query?: CollectionQuery): Promise<CollectionQueryResult> {
    ...
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}?${params}`);
  },

  async getEntry(projectId: string, collectionId: string, id: string): Promise<CollectionEntryResponse> {
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}/${id}`);
  },

  async createEntry(projectId: string, collectionId: string, data: Partial<CollectionEntry>): Promise<CollectionEntryResponse> {
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}`, { method: 'POST', body: data });
  },
  ...
  async getEntryHistory(projectId: string, collectionId: string, id: string, branch?: string): Promise<{ history: any[] }> {
    ...
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}/${id}/history${params}`);
  },
  ...
  async getEntryVersion(projectId: string, collectionId: string, id: string, hash: string, branch?: string): Promise<{ entry: CollectionEntry }> {
    const params = branch ? `?branch=${encodeURIComponent(branch)}` : '';
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}/${id}/history/${hash}${params}`);
  },
  ...
  async previewEntryBranchTransfer(
    ...
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}/${id}/branch-transfer/preview`, {
      method: 'POST',
      body,
    });
  },
  ...
  async applyEntryBranchTransfer(
    ...
    return request(`/api/v1/projects/${projectId}/collections/${collectionId}/${id}/branch-transfer/apply`, {
      method: 'POST',
      body,
    });
  },
